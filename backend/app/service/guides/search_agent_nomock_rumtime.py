...
Simplified without mock complexity
"""
...
subgraphs_dir = current_file.parent
service_dir = subgraphs_dir.parent
...
class SearchAgentLLM:
    ...
    def _initialize_client(self):
        ...
        if self.context.provider == "openai" and api_key:
            ...
            logger.info("SearchAgent using fallback mode")
            self.client = None
    ...
    async def create_search_plan(
        ...
        """
        if not self.client:
            return self._fallback_search_plan(query, keywords)
...
수집 키워드: {', '.join(keywords)}"""
    ...
    async def decide_next_action(
        ...
        """
        if not self.client:
            return self._fallback_next_action(collected_data)
...
데이터 카테고리: {list(collected_data.keys())}"""
    ...
    def _fallback_search_plan(self, query: str, keywords: List[str]) -> Dict[str, Any]:
        """Fallback search plan creation when LLM not available"""
        selected_tools = []
        tool_parameters = {}
    ...
    def _fallback_next_action(self, collected_data: Dict[str, Any]) -> Dict[str, Any]:
        """Fallback next action decision"""
